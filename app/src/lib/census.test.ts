...
describe("buildCensusRows", () => {
  ...
  it("generates only the event when no place is given (residence is null)", () => {
    const { residence, event } = buildCensusRows({
      mediaId: "M-11",
      year: 1950,
      location: null,
    ...
    expect(residence).toBeNull();
    // The event still records that the household appeared in the census.
    expect(event.id).toBe(censusEventId("M-11"));
    expect(event.type).toBe("census");
